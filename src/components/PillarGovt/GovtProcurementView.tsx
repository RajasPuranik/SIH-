  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between gap-3">
        <div className="flex border-b border-slate-200 overflow-x-auto flex-1">
          {tabs.map((tab) => (
            <button
              key={tab.id}
              onClick={() => setActiveSubTab(tab.id)}
              className={`flex items-center gap-1.5 px-4 py-2.5 text-xs font-semibold transition cursor-pointer whitespace-nowrap border-b-2 -mb-px ${
                activeSubTab === tab.id
                  ? `border-b-2 text-slate-900 ${isOfficer ? 'border-blue-500' : 'border-emerald-500'}`
