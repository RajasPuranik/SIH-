...
export const Navbar: React.FC<NavbarProps> = ({ onBlueprintClick }) => {
  ...
  return (
    <header className="sticky top-0 z-40 bg-white border-b border-slate-200">
          ...
          <div className="flex items-center gap-1 sm:gap-1.5">
            ...
            {isLoggedIn && (
              <button
                ...
                className="hidden lg:flex items-center gap-1.5 px-2.5 py-1.5 bg-amber-500 hover:bg-amber-600 text-white text-xs font-semibold rounded-lg transition cursor-pointer"
              >
            ...
            <button
              ...
              className="hidden sm:flex items-center gap-1.5 px-2.5 py-1.5 border border-emerald-300 bg-emerald-50 hover:bg-emerald-100 text-emerald-800 rounded-lg text-xs font-semibold transition cursor-pointer"
            ...
            <button
              ...
              className="hidden lg:flex items-center gap-1.5 px-2.5 py-1.5 border border-indigo-300 bg-indigo-50 hover:bg-indigo-100 text-indigo-800 rounded-lg text-xs font-semibold transition cursor-pointer"
              title="Interactive Voice Response"
            >
              <PhoneCall className="w-3.5 h-3.5 text-indigo-600" />
              <span>Offline IVR</span>
            </button>

            {/* Audio assistant */}
            <button
              onClick={() => setIsAudioAssistantOpen(true)}
              className="hidden lg:flex p-2 rounded-lg text-amber-600 hover:text-amber-800 hover:bg-amber-50 transition cursor-pointer"
            ...
            {/* Language selector */}
            <div className="hidden sm:flex items-center bg-slate-50 border border-slate-200 rounded-lg px-2 py-1.5 gap-1">
              <Globe className="w-3 h-3 text-slate-400 shrink-0" />
              <select
                value={language}
                onChange={(e) => setLanguage(e.target.value as Language)}
                className="bg-transparent text-xs font-medium text-slate-700 focus:outline-none cursor-pointer w-auto"
              >
                {languages.map((l) => (
            ...
            <div className="relative">
              ...
              {showNotifications && (
                <div className="hidden sm:block absolute right-0 top-full mt-1 w-80 bg-white rounded-xl shadow-lg border border-slate-200 z-50">
            ...
            {onBlueprintClick && (
              <button
                ...
                className="hidden lg:flex p-2 rounded-lg text-slate-400 hover:text-slate-700 hover:bg-slate-100 transition cursor-pointer"
            ...
            {/* Profile & logout */}
            {isLoggedIn && (
              <div className="flex items-center gap-0.5 ml-1 sm:pl-2 sm:border-l border-slate-200">
                <button
                ...
                <button
                  onClick={logoutUser}
                  className="hidden sm:flex p-1.5 rounded-lg text-slate-400 hover:text-red-600 hover:bg-red-50 transition cursor-pointer"
                  title="Sign out"
                >
      ...
      {/* Mobile Notification Full-width Panel */}
      {showNotifications && (
        <div className="sm:hidden border-t border-slate-100 bg-white shadow-md">
      ...
      {/* Mobile drawer */}
      {mobileMenuOpen && (
        <div className="lg:hidden border-t border-slate-100 bg-white flex flex-col">
          ...
          <div className="flex flex-col py-2 px-3 gap-1">
            ...
            {userRole === 'mandi_officer' && (
              <button
                onClick={() => { setIsOfficerScannerOpen(true); setMobileMenuOpen(false); }}
                className="sm:hidden flex items-center gap-3 p-3 text-sm font-medium text-slate-700 hover:bg-slate-50 rounded-lg w-full text-left cursor-pointer"
              >
            ...
            {isLoggedIn && (
              <button
                onClick={() => { setLiveStatusOpen(true); setMobileMenuOpen(false); }}
                className="lg:hidden flex items-center gap-3 p-3 text-sm font-medium text-slate-700 hover:bg-slate-50 rounded-lg w-full text-left cursor-pointer"
              >
            ...
            {/* Phone Bot */}
            <button
              onClick={() => { openPhoneBot('inbound'); setMobileMenuOpen(false); }}
              className="sm:hidden flex items-center gap-3 p-3 text-sm font-medium text-slate-700 hover:bg-slate-50 rounded-lg w-full text-left cursor-pointer"
            >
            ...
            {/* IVR Dialpad */}
            <button
              onClick={() => { setIsIVRDialpadOpen(true); setMobileMenuOpen(false); }}
              className="lg:hidden flex items-center gap-3 p-3 text-sm font-medium text-slate-700 hover:bg-slate-50 rounded-lg w-full text-left cursor-pointer"
            >
            ...
            {/* Audio assistant */}
            <button
              onClick={() => { setIsAudioAssistantOpen(true); setMobileMenuOpen(false); }}
              className="lg:hidden flex items-center gap-3 p-3 text-sm font-medium text-slate-700 hover:bg-slate-50 rounded-lg w-full text-left cursor-pointer"
            >
              <div className="w-8 h-8 rounded-full bg-amber-100 text-amber-600 flex items-center justify-center shrink-0">
                <Volume2 className="w-4 h-4" />
            ...
            {/* Language */}
            <div className="sm:hidden flex items-center gap-3 p-3 text-sm font-medium text-slate-700 hover:bg-slate-50 rounded-lg w-full text-left">
              ...
              <select
                value={language}
                onChange={(e) => setLanguage(e.target.value as Language)}
                className="bg-transparent font-medium text-slate-700 focus:outline-none cursor-pointer flex-1 py-1"
              >
                {languages.map((l) => (
                  <option key={l.code} value={l.code}>
                    {l.flag} {l.label}
            ...
            {onBlueprintClick && (
              <button
                onClick={() => { onBlueprintClick(); setMobileMenuOpen(false); }}
                className="lg:hidden flex items-center gap-3 p-3 text-sm font-medium text-slate-700 hover:bg-slate-50 rounded-lg w-full text-left cursor-pointer"
              >
                <div className="w-8 h-8 rounded-full bg-slate-100 text-slate-600 flex items-center justify-center shrink-0">
                  <Layers className="w-4 h-4" />
            ...
            {isLoggedIn && (
              <button
                ...
                className="sm:hidden flex items-center gap-3 p-3 text-sm font-medium text-red-600 hover:bg-red-50 rounded-lg w-full text-left cursor-pointer"
              >
