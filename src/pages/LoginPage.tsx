...
const ROLES = [
  ...
  /* ─── Shared outer wrapper: split-screen on desktop, stacked on mobile ─── */
  const PageShell: React.FC<{ children: React.ReactNode; narrow?: boolean }> = ({ children, narrow }) => (
    <div className="min-h-screen lg:h-screen lg:overflow-hidden bg-slate-50 flex flex-col lg:flex-row">
      ...
      <div className="hidden lg:flex lg:w-[42%] xl:w-[38%] shrink-0 relative bg-emerald-900 text-white flex-col justify-between p-10 xl:p-14 overflow-hidden">
        <div className="absolute inset-0 opacity-10 text-[20rem] leading-none select-none pointer-events-none -right-16 -bottom-24">
          🌾
        </div>
        <div className="relative flex items-center gap-2.5">
          <span className="text-2xl">🌾</span>
          <div>
            <div className="text-lg font-bold tracking-tight leading-none">KisanTrack</div>
        ...
        <p className="relative text-[11px] text-emerald-300/80">
      ...
      <div className="flex-1 flex items-center justify-center p-4 sm:p-6 lg:p-10 lg:overflow-y-auto">
        <div className={`w-full ${narrow ? 'max-w-md' : 'max-w-3xl'}`}>
          ...
          {children}
          <p className="text-center text-[11px] text-slate-400 mt-5 lg:hidden">
            Ministry of Agriculture & Farmers Welfare • APMC Direct Procurement
          </p>
        </div>
      </div>
